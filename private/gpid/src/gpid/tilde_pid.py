...
def objective(sig, hx, hy, dm, dx, dy, reg):
    ...
    B = hx - sig @ hy
    obj = (
        0.5 / np.log(2) * npla.slogdet(np.eye(dm) + hy.T @ hy + B.T @ la.solve(S, B))[1]
    )
    return obj
...
def gradient(sig, hx, hy, dm, dx, dy, reg):
    ...
    g_sig = S_inv_B @ la.solve(
        np.eye(dm) + hy.T @ hy + B.T @ S_inv_B, S_inv_B.T @ sig - hy.T
    )
    return g_sig
...
def project(sig_temp):
    ...
    covxy__m = np.block([[np.eye(dx), sig_temp], [sig_temp.T, np.eye(dy)]])
    ...
    while reg < 1.1:
        ...
        sig_temp_proj = la.solve(
            ...
            la.solve(reg * np.eye(dy) + la.sqrtm(covy__m).real, covxy__m[:dx, dx:].T).T,
        )

        new_covxy__m = np.block(
            [[np.eye(dx), sig_temp_proj], [sig_temp_proj.T, np.eye(dy)]]
        )
...
def exact_tilde_union_info_minimizer(hx, hy, plot=False, ret_obj=False, reg=1e-7):
    ...
    running_obj = []
    if plot:
        running_sig_pre_proj = [
            sig_temp,
        ]
        running_sig_post_proj = [
            sig_temp_proj,
        ]
        running_grad = []
        running_eta = []
    ...
    while True:
        ...
        if len(running_obj) >= patience:
            if extra == 0:
                if (
                    np.abs(np.array(running_obj[-patience:]) - obj) < stop_threshold
                ).all() or i >= max_iterations:
                    if i >= max_iterations:
                        warnings.warn(
                            "Exceeded maximum number of iterations. May not have converged."
                        )
                    if extra_iters == 0:
                        break
                    extra += 1
        ...
        if g_sig_prev is not None:
            sign_changed = (
                -g_sig * g_sig_prev
            )  # -1 if sign did not change, +1 if sign changed
    ...
    if plot:
        ...
        if dx == 1 or dy == 1:
            ...
            post_proj = running_sig_post_proj.reshape(
                (running_sig_post_proj.shape[0], -1)
            )
            plt.plot(post_proj)
            plt.title("Convergence of minimizer")
            plt.ylabel("$\Sigma_i$")
...
def exact_gauss_tilde_pid(
    ...
    imxy = (
        0.5
        * npla.slogdet(
            np.eye(dm) + hxy.T @ la.solve(sigxy + reg * np.eye(*sigxy.shape), hxy)
        )[1]
        / np.log(2)
    )
    ...
    sig, obj = exact_tilde_union_info_minimizer(
        hx, hy, plot=plot, ret_obj=True, reg=reg
    )
