...
def strong_wolfe(fun, x, t, d, f, g, gtd=None, custom=False, **kwargs):
    """
    ...
    if "extra_condition" in kwargs:
        ...
        # use customised strong wolfe line search (with pipeline to avoid nans)
        f, g, t, ls_nevals = _strong_wolfe_custom(
            fun, x.view(-1), t, d.view(-1), f, g.view(-1), gtd, **kwargs
        )
    else:
...
def backtracking(fun, x, t, d, f, g, mu=0.1, decay=0.98, max_ls=500, tmin=1e-5):
    x_new = x.clone()  # Ensure we don't overwrite x accidentally
    f_new = f
    success = False

    t = float(t)  # Ensure t is a Python float to prevent in-place tensor operations

    for i in range(max_ls):
        f_new, x_new = fun(x, t, d)  # Compute new function value and point

        # Armijo condition: f(x + t*d) ≤ f(x) + μ t gᵀ d
        if f_new <= f + mu * t * g.dot(d):
            success = True
            break  # Accept the step

        if t <= tmin:
            warnings.warn("Step size has reached the minimum threshold.")
            break  # Stop search if step size gets too small

        t *= decay  # Reduce step size exponentially

    else:
        warnings.warn("Backtracking did not converge.")  # If loop exits without success

    return x_new, f_new, t, success


def avoid_nans(obj_func, x, t, d):
    f_new, g_new = obj_func(x, t, d)
    while torch.isnan(torch.tensor(f_new)) or torch.isnan(g_new).any():
        t *= 0.8
        # Stop if t gets too small
        if t < 1e-10:
            raise ValueError(
                "Step size too small; unable to find valid function value."
            )
        f_new, g_new = obj_func(x, t, d)
...
def _strong_wolfe_custom(
    ...
    # evaluate objective and gradient using initial step
    #### f_new, g_new = obj_func(x, t, d) ### OLD
    # check for nan values
    f_new, g_new, t = avoid_nans(obj_func, x, t, d)
