...
from .line_search import strong_wolfe, backtracking
...
def _minimize_newton_exact(
    ...
    return_all : bool
        ...
        iterations.
    custom_wolfe : bool
        Use the custom implementation of the strong Wolfe line search.
    ...
    closure = sf.closure
    if line_search == "strong-wolfe" or line_search == "backtracking":
        dir_evaluate = sf.dir_evaluate

    # initial settings
    x = x0.detach().view(-1).clone(memory_format=torch.contiguous_format)
    f, g, _, hess = closure(x)
    if torch.any(hess.diagonal() < 1e-2):
        hess.diagonal().add_(0.1)
    if tikhonov > 0:
    ...
    for n_iter in range(1, max_iter + 1):
        ...
        if line_search == "none":
            ...
            _, _, t, ls_nevals = backtracking(dir_evaluate, x, lr, d, f, g)
            update = d.mul(t)
            x = x + update
        else:
            raise ValueError("invalid line_search option {}.".format(line_search))
...
# Function to evaluate loss along a line
def eval_loss_along_direction(x, d, loss_fn, t_range=(-3, 3), num_points=50):
    """Evaluates loss along the direction d over a range of step sizes"""
    x = x.unsqueeze(0)
    t_values = torch.linspace(t_range[0], t_range[1], num_points)
    losses = []

    for t in t_values:
        x_new = x + t * d
        loss_value = loss_fn(x_new)
        losses.append(loss_value.item())

    return t_values.numpy(), losses


# Example Usage:
# ttt, lll = eval_loss_along_direction(x, d, fun)
# plot(ttt, lll)


def plot(t_values, loss_values):
    # Plot the loss function along the search direction
    plt.plot(t_values, loss_values, label="Loss along search direction")
    plt.axvline(x=0, color="k", linestyle="--", label="Current x")
    plt.xlabel("Step Size t")
    plt.ylabel("Loss")
    plt.legend()
    plt.title("Loss function along descent direction")
    plt.show()
